...
function generatePassword() {
    ...
    const length = 12;
    password.value = Array.from(crypto.getRandomValues(new Uint32Array(length))).map(x => charset[x % charset.length]).join('');
}
...
function copyPassword() {
    ...
    navigator.clipboard.writeText(text).then(() => showAlert1('Пароль скопирован в буфер обмена!'));
}
...
function updateStorageStatus() {
    ...
    warningElement.textContent = usedKB >= MAX_STORAGE * 0.8 ? 'Внимание! Почти все хранилище занято.' : '';
}
    ...
    reader.onload = async function(e) {
        try {
            ...
            if (!encrypted.iv || !encrypted.data) {
                showAlert("Файл не содержит шифрованных данных или повреждён.");
                return;
            }
            ...
            if (!Array.isArray(imported)) {
                showAlert("Файл расшифрован, но формат данных неверен.");
                return;
            }
            ...
            imported.forEach(newEntry => {
                const duplicate = existing.find(
                    e => e.service === newEntry.service && e.login === newEntry.login
                );
            ...
            loadPasswords();
            showAlert("Пароли успешно расшифрованы и загружены!");
        } catch (err) {
            console.error("Ошибка при загрузке:", err);
            showAlert("Ошибка расшифровки. Возможно, неверный пароль или повреждённый файл.");
        }
    };
...
resetBtn.addEventListener('click', () => {
    if (confirm("Вы уверены, что хотите удалить все данные? Это необратимо.")) {
        localStorage.clear();
        location.reload(); // Обновим страницу, чтобы сбросить состояние
    }
});
